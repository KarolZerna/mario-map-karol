...
class World:
    ...
    def discard_successors(self, successors):
        ...
        for successor in successors:
            i = successor[0]
            j = successor[1]
            if 0 <= i < self.rows and  0 <= j < self.columns and self.map[i][j] == self.settings.VISITED:
    ...
    def travel_path_bfs(self, i, j):
        ...
        while open.qsize() != 0:
            state = open.get()
            # mark it as visited
            self.map[state[0]][state[1]] = self.settings.ROAD
            print(self.map, "\n")
    ...
    def number_of_box(self):
        ...
        for i in range(self.rows):
            for j in range(self.columns):
                if self.map[i][j] == self.settings.MARIO:
